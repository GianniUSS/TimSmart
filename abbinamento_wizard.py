...
class AbbinamentoWizard(tk.Toplevel):
    ...
    def _render_step_badge(self):
        ...
        # Campo badge NFC in sola lettura - nessuna tastiera necessaria
    ...
    def _go_step1(self):
        ...
        if hasattr(self, 'master') and hasattr(self.master, 'hide_keyboard'):
            self.master.hide_keyboard()
        self.step = 1
        self._render_step()
    ...
    def _on_badge(self, badge_id: str):
        def ui():
            # Abilita temporaneamente il campo per aggiornarlo
            if self._entry_badge and self._entry_badge.winfo_exists():
                try:
                    self._entry_badge.config(state='normal')
                    self.badge_var.set(badge_id)
                    self._entry_badge.config(state='readonly')
                except Exception:
                    self.badge_var.set(badge_id)
            else:
                self.badge_var.set(badge_id)
    ...
    def _save_badge(self):
        ...
        print(f"[DEBUG] Badge {badge} abbinato con successo a {self.nome_var.get()}")
        if ok:
        try:
            print("[DEBUG] Tentativo di mostrare msgbox di successo...")
            self._show_message('info', 'Salvato', 'Abbinamento salvato con successo!')
            print("[DEBUG] Msgbox mostrata con successo")
        except Exception as e:
            print(f"[DEBUG] Errore nella msgbox: {e}")
            self._show_status("Abbinamento salvato", error=False)
        self.after(0, self._cancel)
        else:
        try:
            print("[DEBUG] Tentativo di mostrare msgbox di errore...")
            self._show_message('error', 'Errore', 'Errore durante il salvataggio')
            print("[DEBUG] Msgbox di errore mostrata")
        except Exception as e:
            print(f"[DEBUG] Errore nella msgbox di errore: {e}")
    ...
    def _show_message(self, kind: str, title: str, text: str):
        """Mostra un messaggio touch-friendly sempre in primo piano"""
        try:
        ...
        try:
            # Crea dialog sempre in primo piano
            dlg = tk.Toplevel(self)
            dlg.transient(self)
            dlg.attributes('-topmost', True)
            dlg.resizable(False, False)
            dlg.configure(bg='#FFFFFF')
            dlg.overrideredirect(False)  # Mantiene bordo finestra
            
            # Forza sempre in primo piano
            dlg.lift()
            dlg.attributes('-topmost', True)
            dlg.focus_force()
            ...
            width = max(480, min(720, int(vw * 0.55)))  # Più largo per touch
            
            # Header più grande per touch
            header = tk.Frame(dlg, bg=header_bg)
            header.pack(fill='x')
            tk.Label(header, text=title, font=('Segoe UI', s(28)), fg='#FFFFFF', bg=header_bg, 
                    padx=s(24), pady=s(16)).pack(anchor='w')

            # Body con più padding per leggibilità
            body = tk.Frame(dlg, bg='#FFFFFF')
            body.pack(fill='both', expand=True, padx=s(32), pady=s(24))
            tk.Label(body, text=text, font=('Segoe UI', s(20)), fg='#374151', bg='#FFFFFF', 
                    justify='left', wraplength=width - s(64)).pack(anchor='w')

            # Pulsante OK più grande e touch-friendly
            btn = tk.Button(
                body,
                text='OK',
                ...
                bd=3,
                cursor='hand2',
                command=lambda: self._safe_close_dialog(dlg)
            )
            btn.pack(fill='x', pady=(s(32), 0), ipadx=s(20), ipady=s(18))
            ...
            # Configurazioni finali per primo piano
            try:
                dlg.grab_set()
                dlg.lift()
                dlg.attributes('-topmost', True)
                ...
                # Binding touch-friendly
                dlg.bind('<Escape>', lambda e: self._safe_close_dialog(dlg))
                dlg.bind('<Return>', lambda e: self._safe_close_dialog(dlg))
                btn.bind('<Button-1>', lambda e: self._safe_close_dialog(dlg))
                
                # Assicura che rimanga sempre visibile
                def keep_on_top():
                    try:
                        if dlg.winfo_exists():
                            dlg.lift()
                            dlg.attributes('-topmost', True)
                            dlg.after(100, keep_on_top)
                    except:
                        pass
                dlg.after(50, keep_on_top)
            ...
            try:
                ...
                tkinter.messagebox.showinfo(title, text, parent=self)
            except Exception:
                pass
    
    def _safe_close_dialog(self, dialog):
        """Chiude il dialog in modo sicuro"""
        try:
            dialog.grab_release()
        except:
            pass
